...
export const ThemeToggle = () => {
    ...
    return (
        <Switch checked={isLight} onChange={() => setTheme(isLight ? 'dark' : 'light')} className='group relative flex h-8 w-14 cursor-pointer rounded-full bg-palette-gray-4 p-1 transition-colors outline-none'>
            <span aria-hidden='true' className='pointer-events-none flex justify-center items-center size-6 translate-x-0 rounded-full bg-palette-background p-1 transition group-data-[checked]:translate-x-6'>
